...
typedef enum {
    ...
    ROLLING_BACK,
    PULLING_GOLD,
    dynamite_MOVING,    // dynamite is moving toward the pulled object
    dynamite_EXPLOSION   // explosion effect (explosion.png) shown
} HookState;
...
int main(int argc, char* argv[])
{
    ...
    TTF_Font* font48 = TTF_OpenFont("arial.ttf", 48);
    if (!font48) {
        printf("Failed to load font (48pt): %s\n", TTF_GetError());
        return 1;
    }

    // ------------------- Create window (1366 x 768) -------------------
    SDL_Window* window = SDL_CreateWindow("Đào vàng",
    ...
    while (!exitProgram) {
        ...
        if (menuResult == 1) { // if menu returns quit signal
            exitProgram = true;
            break;
        }

        // ------------------- Initialize Game Session Variables -------------------
        // (Reinitialize session-specific variables each time.)
        // Character is placed at center with feet around 170px from top.
        ...
        if (score >= 400) {
            // Win: show success.png for 4 sec, then load shop.png.
            SDL_RenderClear(renderer);
            SDL_RenderCopy(renderer, successTexture, NULL, &fullScreenRect);
            SDL_RenderPresent(renderer);
            SDL_Delay(4000);
            SDL_Surface* shopSurface = IMG_Load("shop.png");
            if (!shopSurface)
                printf("Error loading shop.png: %s\n", IMG_GetError());
            SDL_Texture* shopTexture = SDL_CreateTextureFromSurface(renderer, shopSurface);
            SDL_FreeSurface(shopSurface);
            bool shopDone = false;
            while (!shopDone) {
                SDL_Event shopEvent;
                while (SDL_PollEvent(&shopEvent)) {
                    if (shopEvent.type == SDL_QUIT ||
                        (shopEvent.type == SDL_KEYDOWN && shopEvent.key.keysym.sym == SDLK_ESCAPE))
                        shopDone = true;
                }
                SDL_RenderClear(renderer);
                SDL_RenderCopy(renderer, shopTexture, NULL, NULL);
                SDL_RenderPresent(renderer);
                SDL_Delay(16);
            }
            SDL_DestroyTexture(shopTexture);
        } else {
            // Fail: show failure.png for 4 sec.
            SDL_RenderClear(renderer);
            SDL_RenderCopy(renderer, failureTexture, NULL, &fullScreenRect);
            SDL_RenderPresent(renderer);
            SDL_Delay(4000);
        }
        // End of one game session; loop back to menu.
